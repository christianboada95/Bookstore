	...
	public ArrayList<Book> run(Integer n) {
		...
		for (int i = 0; i < n; i++) {
			...
			for (int j = 0; j < categories.length; j++) {
				...
				categories[j] = categoryList.get(
						faker.number().numberBetween(0, categoryList.size()));
			...
			Publisher ps = publisherRepository.findById(
					(int) faker.number().numberBetween(1, max)).get();
			// select random author
			max = authorRepository.count();
			Author at = authorRepository.findById(
					(int) faker.number().numberBetween(1, max)).get();
